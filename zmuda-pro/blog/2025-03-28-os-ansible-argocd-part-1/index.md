...
5. Lastly, we create a preconfiguration file so that the installation process is automated.
   You can find an example file in the [Debian documentation](https://www.debian.org/releases/bookworm/example-preseed.txt).
   My preconfiguration file looks like this:
   ```bash title="/preseed-server.conf"
   #_preseed_V1
   ...
   # Add ssh key for my user
   d-i preseed/late_command string in-target mkdir -p /home/adzik/.ssh; \
   in-target /bin/sh -c "echo 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIKk7j5NrdCVSvPiDBoqUM/VC5ltpWjXRqEgCyjptugmp adam@zmuda.pro' >> /home/adzik/.ssh/authorized_keys"; \
   in-target chown -R adzik:adzik /home/adzik/.ssh/; \
   in-target chmod 644 /home/adzik/.ssh/authorized_keys; \
   in-target chmod 700 /home/adzik/.ssh/
   ```
